...
export async function getPetStatus(userId) {
    ...
    return { name: pet.name, hp: `${pet.hp}/${pet.maxHp}`, status: lifePhase, fashion_status: pet.fashion_status, accessories: pet.accessories };
}
...
export async function feedPet(foodName, userId) {
    ...
    await pet.save();
    return { message: `Alimentaste a ${pet.name} con ${food.name}.`, newStatus: await getPetStatus(userId) };
}
...
export async function giveMedicine(treatmentType, userId) {
    ...
    if (!treatment) throw new Error("Tratamiento no válido: 'basic' o 'advanced'.");
